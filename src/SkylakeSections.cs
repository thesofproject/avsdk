...
namespace itt
{
    ...
    public class SectionSkylakeTokens : SectionVendorTokens
    {
            ...
            : base(IDENTIFIER)
        {
            ...
            Tokens = values.Select(t => Tuple.Create(t.GetName(), (uint)t)).ToArray();
        }
    }
    ...
    public static class SectionHelper
    {
                                                            ...
                                                            SKL_BLOCK_TYPE type = SKL_BLOCK_TYPE.TUPLE)
        {
            ...
            bytes.Tuples = new[]
            {
                Tuple.Create(SKL_TKN.U8_BLOCK_TYPE.GetName(), (byte)type)
            };

            var shorts = new VendorTuples<ushort>("u16_size_desc");
            shorts.Tuples = new[]
            {
                Tuple.Create(SKL_TKN.U16_BLOCK_SIZE.GetName(), (ushort)size)
            };
        ...
        public static SectionVendorTuples GetNumDescriptor(this Section section, int num)
        {
            ...
            bytes.Tuples = new[]
            {
                Tuple.Create(SKL_TKN.U8_NUM_BLOCKS.GetName(), (byte)num)
            };
